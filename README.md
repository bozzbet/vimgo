...
When no worker name is provided, the installer will generate one in the format `YOUR_WALLET.iVim-<deviceID>`. The device ID comes from the Android serial value, which is usually the same value shown by `adb devices`.
